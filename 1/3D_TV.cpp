...
int main(int argc, char** argv)
{
	...
	capture = cvCaptureFromAVI("D://Homework//研究影片//02.avi");
	...
	while(true)	{	
   		if(cvGrabFrame(capture)){
				...
				for( j=0 ; j < width ; j++ ){	
					if ((i%2)==1 &&(j%2)==1){
						if ((frame_counter%4) == 0){
							...
							motion_diff0[i][j] = abs(frame_A->imageData[k*step+l*3+1] - frame_B->imageData[k*step+l*3+1]);
							motion_diff1[i][j] = abs(frame_A->imageData[k*step+l*3+1] - frame_C->imageData[k*step+l*3+1]);
							motion_diff2[i][j] = abs(frame_A->imageData[k*step+l*3+1] - frame_D->imageData[k*step+l*3+1]);
							...
							motion_diff0[i][j] = abs(frame_B->imageData[k*step+l*3+1] - frame_C->imageData[k*step+l*3+1]);
							motion_diff1[i][j] = abs(frame_B->imageData[k*step+l*3+1] - frame_D->imageData[k*step+l*3+1]);
							motion_diff2[i][j] = abs(frame_B->imageData[k*step+l*3+1] - frame_A->imageData[k*step+l*3+1]);
							...
							motion_diff0[i][j] = abs(frame_C->imageData[k*step+l*3+1] - frame_D->imageData[k*step+l*3+1]);
							motion_diff1[i][j] = abs(frame_C->imageData[k*step+l*3+1] - frame_A->imageData[k*step+l*3+1]);
							motion_diff2[i][j] = abs(frame_C->imageData[k*step+l*3+1] - frame_B->imageData[k*step+l*3+1]);
							...
							motion_diff0[i][j] = abs(frame_D->imageData[k*step+l*3+1] - frame_A->imageData[k*step+l*3+1]);
							motion_diff1[i][j] = abs(frame_D->imageData[k*step+l*3+1] - frame_B->imageData[k*step+l*3+1]);
							motion_diff2[i][j] = abs(frame_D->imageData[k*step+l*3+1] - frame_C->imageData[k*step+l*3+1]);
						...
						if (k>5 && l>3){
							...
							if ((binary_imgA[k][l]+binary_imgB[k][l]+binary_imgC[k][l]+binary_imgD[k][l])>4){
								Dilation_img[i][j] = 255;
								Dilation_img[i-1][j-1] = 255;
			...
			for( j=0 ; j < width ; j++ ){	
				...
				frame_tt->imageData[i*step+j*3+0] = D_reg[i][j];//B
				frame_tt->imageData[i*step+j*3+1] = D_reg[i][j];//G
				frame_tt->imageData[i*step+j*3+2] = D_reg[i][j];//R 
				
				//frame_tt->imageData[i*step+j*3+0] = depth_floor[i][j];//B
				//frame_tt->imageData[i*step+j*3+1] = depth_floor[i][j];//G
				//frame_tt->imageData[i*step+j*3+2] = depth_floor[i][j];//R
			}
			///存影像
			cvWriteFrame(writer,frame_tt);
